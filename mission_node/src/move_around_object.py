...
class GoAroundObject(smach.StateMachine):
    def __init__(self, shared_state_data, object_name):
        ...
        # shared_data is initialized inside the initialize_subscribers() function
        ...
        with self:
            smach.StateMachine.add('move_left_object',
                                    ...
                                    transitions={'success': 'move_right_object', 'aborted': 'failure', 'edge_case_detected': 'failure', 'object_not_detected': 'failure'})
            smach.StateMachine.add('move_right_object', ContinuePoseObjectMovement(edge_case_callback=movement_edge_cases, offset_point=Point(x=1, y=0, z=0)),
